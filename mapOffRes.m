...
function offResMap = mapOffRes( dataCube, TEs, varargin )
  % offResMap = mapOffRes( dataCube, TEs [, 'mask', mask ] )
  %
  % Inputs:
  ...
  unwrapped = unwrap_phase( mask .* angleOffset );
  unwrapped = unwrapped - median( unwrapped( mask == 1 ) );
  offResMap = unwrapped / ( TEs(2) - TEs(1) );
  if numel( mask ) > 0, offResMap = offResMap .* mask; end
