...
function [M0Img,T1Img] = estimateM0T1Img( I1, I2, time1, time2, alpha1, alpha2, magMask )

  function out = fModel( x )
    M0=x(1);
    T1=x(2);
    model1 = M0 * ( 1 - exp(-time1/T1) ) * sin( alpha1 );
    model2 = M0 * sin( alpha2 ) * ( ...
      ( 1 - exp( -time1/T1 ) ) * cos( alpha1 ) * exp( -time2 / T1 ) + ...
      ( 1 - exp( -(time2-time1)/T1 ) ) );
